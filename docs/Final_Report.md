...
| RR-14 | Labelled-data budget falls below the proposal's 100-image target; unlabelled corpus collection blocks SSP/UDA | small-data triage; Section 9.6 challenge 6 | High (confirmed) | High | Layered response: 50-image auto-labelled seed (zero human cost) → 8-image hand-corrected ground truth (Roboflow) → ScreenSpot import for the per-instruction grounding row (n = 334) → `scripts/auto_capture_corpus.py` for passive unlabelled accumulation. Each tier is reported alongside its protocol caveat so the dissertation's claims stay calibrated to the data actually held. | Mitigated |

Three observations about the register are worth pulling out for dissertation prose.

First, **the highest-impact risks are all data-quality risks**, not modelling or deployment risks. RR-01, RR-02, RR-10 and RR-14 between them account for the project's four biggest empirical findings (the auto-label/hand-correct mAP gap, the recall-bounded source-domain backbone, the icon class-distribution skew, and the small-data triage pattern that determined which mAP and CPV protocols the dissertation could report). Each is a reminder that the modelling chain is no stronger than its weakest data link.
...
A sixth challenge, different in kind from the five above, ran across the whole project rather than appearing as a single incident: **the small-data budget and the layered response it forced.** The proposal committed to a 100-image hand-labelled desktop corpus and a 2,000-image unlabelled corpus. Neither budget was realistic for a single-developer MSc project with no annotation labour, no paid annotation platform, and Colab Free as the only compute. The response was deliberately layered rather than substitutive, and the layering is what unlocked each of the dissertation's empirical claims at the size of data actually available. Auto-labelling a 50-image desktop seed with the source-domain detector produced a starting point at zero human cost, and is the M0-M3 backbone of the transfer-learning matrix in Section 7.2. Hand-correcting 8 of those images in Roboflow produced the 356-box ground-truth pool that every headline mAP and per-element CPV number cites, and is the work that exposed the auto-label-equals-ground-truth fallacy discussed above as challenge three. When the hand-correction expansion toward 100 images stalled, the data side of gap D-07 was closed not by paying for more annotation but by importing a third-party-labelled public benchmark, ScreenSpot, which contributed the 334-row desktop slice and the 57.5 per cent per-instruction CPV number now reported in Section 7.3.1. When the self-supervised and unsupervised adaptation arms of the experimental matrix (gaps D-02 through D-04) required an unlabelled corpus, the response was a background auto-capture script (`scripts/auto_capture_corpus.py`) that buckets one screenshot per minute by foreground-application name and accumulates the corpus passively over normal computer use rather than through dedicated capture sessions. None of these is a workaround in the dismissive sense of the word; each is a deliberate methodological choice at the point where the proposal asked for "more data." The pattern that emerged, in the author's own framing, was *third-party data first, hand-labelled data second, bespoke capture last*: at every step, the project asked whether the same evidence could come from already-existing public sources before committing scarce human time to bespoke collection. The pattern cost about a week of design time across the project and saved roughly a month of annotation labour, at the cost of a small narrative complication in the evaluation chapter — both protocols (per-element recall on the hand-corrected set; per-instruction grounding success on ScreenSpot) have to be reported together with the protocol caveat in Section 8.2.

## 9.7 Limitations

The dissertation's limitations cluster into seven categories.

The first is **the labelled-target-data ceiling**. The hand-corrected ground-truth pool is 8 images, carrying 356 exhaustively-labelled bounding boxes. Every detector mAP and per-element CPV number reported against this pool is therefore a small-sample number with a wide confidence interval. The hand-correction expansion toward the proposal's 100-image target is recorded as gap D-07; Phase 3 of the project closed the *evidence* side of that gap by adding a 334-row evaluation against the public ScreenSpot benchmark (Section 7.3.1) that provides per-instruction grounding success on third-party-labelled data, but the per-element-recall ceiling on Win11 native screens remains a directional rather than confidently-precise figure until the hand-correction work continues. The honest reading is that the 1.4 per cent hand-corrected CPV is a small-n signal and the 57.5 per cent ScreenSpot CPV is the better-sampled one; the two should be cited together with the protocol caveat from Section 8.2. The companion path for unlabelled data — needed for the self-supervised pre-training lane (gap D-02) — is no longer gated on dedicated capture sessions: `scripts/auto_capture_corpus.py` accumulates one screenshot per minute over normal computer use, bucketed by foreground-application name, which removes the manual-capture-session bottleneck that would otherwise gate gap D-06 toward the proposal's 2,000-image unlabelled target.
