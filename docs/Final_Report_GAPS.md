...
|----|-----|---------|--------|--------|
| D-01 | DETR-R50 backbone trained on CLAY and evaluated zero-shot on the labelled desktop test set. Proposal Section 7.B requires both YOLOv8 and DETR for RQ3. Currently only YOLOv8 numbers exist. | Ch6, Ch7, Ch8 | new Colab notebook `09_detr_source.ipynb` + `10_detr_finetune.ipynb` | **SOURCE-SIDE DONE; TARGET-SIDE PENDING** — Phase 4.2, 2 June 2026. `09_detr_source.ipynb` ran fresh with the compute-fit config: 6 epochs, imgsz cap {600, 800}, fp16 autocast + GradScaler, micro-batch 2 / grad-accum 8 (effective 16). Result on CLAY test split: **mAP@0.5 = 0.2438, mAP@0.5:0.95 = 0.1606** (`reports/tables/source_domain_results_detr.csv`). Compared to YOLOv8s (`source_domain_results.csv`: mAP@0.5 = 0.4505, mAP@0.5:0.95 = 0.3498 at 30 epochs imgsz=640 fp32), DETR-R50 reaches ~54 % of YOLOv8's mAP@0.5 while consuming ~6× the wall-clock time per training run. **Methodological context for Section 8.x:** this is a Colab-Free-matched comparison — the first DETR attempt at 8 epochs imgsz=1333 fp32 disconnected mid-training at ~2.5 epochs (mAP@0.5 = 0.0114, retracted); the 6-epoch fp16 config was chosen specifically so 6 epochs fit one Colab Free session with margin. Target-side evaluation (zero-shot on the 8-image hand-corrected desktop set, plus optional head-only fine-tune) is still pending via `10_detr_finetune.ipynb`. |
| D-02 | SSP (self-supervised pre-training) experiment: train both backbones with the generative inpainting task on the ~2,000 unlabelled corpus, then fine-tune. Phase 2 Experiment 2 in proposal. | Ch6, Ch7, Ch8 | new notebooks `11_ssp_pretrain.ipynb` + `12_ssp_finetune.ipynb` | **IN PROGRESS** — Phase 4.4 sub-phase opened 26 May 2026; depends on D-06; SimCLR-style contrastive pretraining on the CSPDarknet backbone planned (replaces the proposal's "generative inpainting" — simpler to implement on Colab Free). |
...
| D-06 | Unlabelled desktop corpus expansion from 50 (current) to ~2000 screenshots as the proposal specifies. Required for D-02, D-03, D-04. | Ch6 | `scripts/capture_screenshots.py` extended to walk 10-15 apps and capture systematically | **IN PROGRESS** — Phase 4.1 sub-phase opened 26 May 2026; new background script `scripts/auto_capture_corpus.py` written, captures every 60 s with foreground-app bucketing. Target ≥1500 screens across ≥10 apps over 1-2 calendar days of passive accumulation. |
...
| T-02 | Table 7.x: sample-efficiency table: mAP@0.5 vs $k$ labelled images, both with and without SSP. | Ch7 | new CSV `reports/tables/sample_efficiency.csv` from D-05 + D-02 | **PARTIAL (no-SSP side DONE)** — D-05 numbers are in `reports/tables/sample_efficiency.csv` as of 3 June 2026; SSP comparison columns still depend on D-02. |
...
| ID | Gap | Chapter | Source | Status |
|----|-----|---------|--------|--------|
| W-01 | References list at end of report. Currently inline `[N]` citations use the proposal's numbering. Need a single sequential list ordered by first appearance in dissertation. | end of report | mechanical pass once all chapters written | **DONE** — Phase 5/6, 14 May 2026. Reproducible script `scripts/renumber_references.py` walks the body, collects every `[N]` and `[LN]` citation skipping fenced-code regions, collapses the 5 alias entries (L1=[8], L3=[1], L4=[10], L7=[33], L9=[9]) onto their canonical numeric entries, re-numbers in first-appearance order, and rewrites both the inline citations and the references list. Result: 53 canonical references (was 58 entries with aliases), 92 inline citation groups preserved, list is `[1]..[53]` with no gaps. Idempotent: re-running after future citation edits regenerates cleanly. |
| W-02 | Chapter 6 (Implementation) sections covering DETR backbone implementation, SSP pretraining objective, and UDA framework code paths. These exist as plans in the proposal but are not yet implemented; corresponding prose must follow the code. | Ch6 | depends on D-01..D-04 | **PARTIAL** — Phase 5/6, 14 May 2026. Writing-sync pass applied: Section 6.12 now describes the two specialised CPV scripts (`run_cpv.py`, `run_cpv_screenspot.py`) as part of the evaluation harness. Sections 7.3.1, 8.2, 8.8, 9.7, 9.8 fold in the Phase 2 hand-corrected CPV (1.4 %) and Phase 3 ScreenSpot CPV (57.5 %) numbers with the explicit per-element-recall vs per-instruction-grounding-success protocol caveat. The DETR / SSP / UDA sub-sections in Chapter 6 (6.5.5-6.5.8) remain stub paragraphs marked "pending: D-01..D-04" — accurate as written because those experiments are not run. Full closure of W-02 requires D-01..D-04 to run first. |
| W-03 | Chapter 9 Section 9.3 "Learning Outcomes" and Section 9.4 "Highly Relevant Academic Modules" need personal-voice reflections that only the author can provide (which modules from the MSc curriculum mapped to which parts of the work, what surprised the author during execution, etc.). | Ch9 | author-owned | **PARTIAL** — Phase 5/6, 14 May 2026. Section 9.3 "Personal development" paragraph drafted in the author's voice using the actual project pivots (Phase 3 hand-correction-to-ScreenSpot pivot; auto-label-to-hand-corrected mAP retraction; ScreenSpot bbox-format bug). Section 9.4 "Highly Relevant Academic Modules" drafted as five module clusters, each mapped to the chapter where its content shows up. Institution placeholder filled from the report's title page (MSc Data Science, Robert Gordon University Aberdeen / Informatics Institute of Technology (IIT)). Five module-name placeholders remain pending the author confirming exact handbook titles: `[MODULE: Machine Learning / Deep Learning Foundations]`, `[MODULE: Computer Vision / Image Analysis]`, `[MODULE: Research Methods / Dissertation Project]`, `[MODULE: Software Engineering / Software Architecture]`, `[MODULE: Ethics / Professional Practice]`. Author should replace each placeholder with the exact handbook title at submission time. |
| W-04 | Chapter 9 Section 9.5 "Self-Taught Areas and New Skills": same as W-03, author-owned. | Ch9 | author-owned | **DONE** — Phase 5/6, 14 May 2026. Section 9.5 expanded from four self-taught areas to five. Third (AI-detection-register writing) placeholder paragraph filled with the author's reflection on skill durability. New fifth area added: public-benchmark integration workflow, anchored to the ScreenSpot integration in Phase 3 and the bbox-format bug, MAX_PATH lockfile, and HF cache routing lessons. |
