...
| D-07 | Labelled desktop corpus expansion from 8 (current hand-corrected test) to 100 (proposal target). Required for D-05 to be meaningful. | Ch6 | CVAT or Roboflow annotation session **or** evaluate against a public benchmark (preferred). | **PLANNED — Phase 3.** Path switched from hand-annotating 100 desktop screens to evaluating the same ONNX detector against the **ScreenSpot** desktop slice (Cheng et al. 2024, SeeClick paper) — ~280-310 real Windows/macOS screens with third-party-labelled bboxes. CPV is class-agnostic so taxonomy mismatch is harmless. Script: `scripts/run_cpv_screenspot.py`. The 8-image hand-corrected set still backs the *class-aware* mAP and per-class CPV; ScreenSpot supplies the *larger-N, out-of-domain* row. Methodologically stronger than self-labelled, and citable. |
