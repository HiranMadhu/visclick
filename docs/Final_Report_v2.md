...
| R-NFR-06 | Maintainability | Object-oriented modular package (`visclick.{capture, detect, ocr, match, act, bot, gui}`) built around 8 substantive classes (`Capture`, `Detector`, `OCREngine`, `Matcher`, `Actor`, `Bot`, `BotResult`, `VisClickApp`) plus 2 PyTorch subclasses in the training script and a 4-class `Baseline` hierarchy in the evaluation harness; composition-over-inheritance; every module PEP-8 clean | 8 pipeline / GUI classes + 4 baseline subclasses + `Baseline` ABC + `BaselineResult` dataclass + 2 PyTorch subclasses = 16 classes total; `ruff check` clean; class diagrams in Figures 12A and 12B | source review + Figures 12A, 12B | FULL |
| R-NFR-07 | Extensibility | Three extension seams. (i) New baseline methods plug in by subclassing `Baseline` (in `scripts/baseline_common.py`) and implementing `predict(image_rgb, instruction) -> BaselineResult`; the module-level `predict()` delegate keeps the older function-style contract working too. (ii) Any of the five pipeline collaborators can be replaced by dependency injection through `Bot.__init__(capture=, detector=, ocr=, matcher=, actor=)` without touching the orchestrator body. (iii) A new OCR backend or matcher scoring policy can be introduced by subclassing `OCREngine` or `Matcher` and passing the subclass instance to `Bot`. | 4 baseline subclasses + injectable-collaborator constructor + ABC-based extension pattern | `scripts/baseline_*.py`; `src/visclick/bot.py::Bot.__init__`; Figure 12B | FULL |
...
A design decision worth surfacing. Alongside the classes, each pipeline module still exposes its public operations as module-level functions (for example `visclick.capture.grab`, `visclick.match.best_box`). These are one-line delegates to a module-scope default instance of the corresponding class. The pattern is deliberate. It keeps existing scripts, notebooks and the four baseline adapters working without modification, and it gives callers a choice between the classical stateless-function style and the injectable object-oriented style. Neither style is claimed as superior; the class-based style is used inside the orchestrator and inside `baseline_visclick.py`, and the function-based style is used inside the notebooks. Both styles resolve to the same instance data.

### 5.5.2 BASELINE CLASS HIERARCHY

The four click methods that Chapter 7 compares side by side — SikuliX-style template matching, OCR-only text grounding, the Windows accessibility-tree method, and the full VisClick pipeline — are wired through a small abstract base class rather than through a duck-typed function contract. Figure 12B shows the resulting class hierarchy.

[FIGURE 12B: Baseline class hierarchy.
Source: `docs/figures/figure_12B_baseline_hierarchy.png` (produced from `docs/figures/figure_12B_baseline_hierarchy.svg`; UML diagram with an abstract Baseline base and four concrete subclasses).
Caption: Baseline class hierarchy. A single `Baseline` abstract base class declares the `predict(image_rgb, instruction, **kwargs) -> BaselineResult` contract; four concrete subclasses (`TemplateBaseline`, `OCRBaseline`, `PyWinAutoBaseline`, `VisClickBaseline`) implement the contract; each returns the same `BaselineResult` value object which the evaluation runner writes to CSV.]

The `Baseline` abstract base class lives in `scripts/baseline_common.py` and declares the contract:
...
class Baseline(ABC):
    ...
    def predict(self, image_rgb, instruction, *,
                offset=(0, 0), **kwargs) -> BaselineResult: ...
```

Each of the four concrete baselines subclasses this ABC in its own file (`baseline_template.py`, `baseline_ocr_only.py`, `baseline_pywinauto.py`, `baseline_visclick.py`), sets `name` to the string that the CSV column will carry, and implements `predict`. Per-method configuration lives on the instance: `TemplateBaseline` carries the templates directory and the correlation threshold, `OCRBaseline` carries the engine choice and the fuzzy-match cut-off, `VisClickBaseline` carries the ONNX weights path and lazily loads a `Detector` on first predict so a 15-task run pays the model-load cost once rather than fifteen times. `PyWinAutoBaseline` is stateless because the UIA descendant search takes its target from the task-specific kwargs, not from instance state.

The evaluation runner in `scripts/run_baselines.py` was written against the earlier function-style contract, so each baseline module also exposes a bare `predict(...)` function that delegates to a module-scope default instance. The runner keeps working unchanged, and any future author who wants to add a fifth baseline (say a MLLM-grounded method) can either subclass `Baseline` and register the class or drop a `predict()` function in a new file, whichever suits the method. The R-NFR-07 (Extensibility) requirement is satisfied by both seams.
