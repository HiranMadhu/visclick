...
> **How to use this file.** This is the continuation of `Final_Report_v2.md`. It contains Chapters 6-9 of the v2 draft, kept separate so the source files stay editable without churning a single 1,500-line markdown blob. Same conventions apply: ALL CAPS chapter and section headings, sequential figure and table numbering carrying on from Part 1 (last figure was Figure 13; last table was Table 5), Harvard "Cite Them Right" citations, natural-informality voice. Every code snippet in this file is preceded by a short "Why this snippet is shown" justification so that no code block appears without a reason to cite it. At submission time the two markdowns are concatenated and converted to Word as a single document.
...
This chapter is the build half of the design-build-evaluate loop and the implementation counterpart of the design chapter. It walks through the implementation in roughly the order Chapter 5 introduced the components. It begins with the development environment, because every implementation claim that follows depends on having that environment reproducibly available. From there it covers the dataset implementation (acquisition, class collapse, the auto-label and hand-correction passes), the source-domain detector (YOLOv8s and DETR-R50), the four adaptation methods (few-shot fine-tune, self-supervised pre-training, Adaptive Teacher, SHOT), the prototype package, and the evaluation harness. The chapter closes with what was deliberately left out and where to find it.

Two conventions are worth flagging up front. First, every code snippet in this chapter is preceded by a short "Why this snippet is shown" justification, so that no code block appears without a stated reason for its inclusion. Second, Section 6.7.7 includes an actual screenshot of the running GUI (Figure 14), which pairs with the wireframe from Section 5.8 to make the design-versus-implementation distinction concrete: the wireframe is the intent, the screenshot is the delivered artefact.
...
**Why this snippet is shown.** The training cell below is the smallest artefact that captures the full source-domain training configuration. Every hyperparameter that materially affects the reported mAP number (30 epochs, `imgsz=640`, `batch=16`, `seed=42`) is visible here; a reproduction attempt can start by copying this block into a fresh notebook.
...
**Why this snippet is shown.** The block below is quoted because it is the smallest artefact that shows why DETR is the more expensive architecture under this project's compute budget. The `ignore_mismatched_sizes=True` argument, the micro-batch-plus-gradient-accumulation split, and the differential learning rates for head and backbone are all consequences of the T4's 16 GB VRAM cap; a reader who wonders why DETR training took six times longer than YOLOv8s training can see the answer directly.
...
**Why this snippet is shown.** The block below is the deployed model's training recipe. The `freeze=10` argument is the pivotal decision - it is what separates a "head fine-tune" from a "full fine-tune" and is the single most influential hyperparameter for the sample-efficiency curve reported below. Quoting this cell makes the freeze decision auditable.
...
**Why this snippet is shown.** The one-line export command is quoted because it is the smallest artefact that reveals the deployment target. `opset=12` is the highest ONNX opset supported by the `onnxruntime` build shipped with the prototype; `simplify=True` collapses redundant shape operations and is what keeps the deployed file at approximately 45 MB rather than the 65 MB the unsimplified export produces. A reader who wonders how the training weights become the deployable ONNX file has the answer in this one line.
...
The capture module wraps `mss` and exposes two entry points: `capture_monitor(idx)` and `list_monitors()`. The non-trivial work is the multi-monitor offset propagation, the source of observation O13 (the cursor moving by a few pixels onto the wrong monitor). The fix is to return the chosen monitor's `(left, top)` offset alongside the image, and to thread that offset through every downstream layer that maps a box back to a click coordinate.

**Why this snippet is shown.** The signature below is short enough to quote and is the smallest code artefact that reveals the multi-monitor design decision. The critical detail is that the function returns a tuple, not just an image; a reader who only looks at the signature can see immediately that the offset is an explicit return value rather than a hidden global.
...
The detect module loads the ONNX file at start-up and runs `onnxruntime` inference per call.

**Why this snippet is shown.** The two-line class signature below is quoted because it is the only place in the pipeline that uses a class rather than a plain function (see §4.4.2 on the paradigm choice). The class is not there to introduce inheritance - there is none - it is there because `onnxruntime.InferenceSession` is expensive to construct and its lifetime has to be bound to something. This snippet is the tight demonstration that the codebase uses OOP only when the language forces it to, and nowhere else.
...
The OCR module is the most complex single file in the codebase. It has to handle two backend mismatches - EasyOCR's `readtext` returns `(bbox, text, conf)` triples while pytesseract's `image_to_data` returns a DataFrame - and a fallback path that runs when neither backend is available.

**Why this snippet is shown.** The public surface is quoted because it is the smallest artefact that reveals the module's single-responsibility split. Four functions, four responsibilities: recognise text globally, recognise text in one region, ground a natural-language query, self-check at start-up. Any addition to this file that would not slot cleanly under one of these four names is the wrong shape and should live somewhere else.
...
The orchestrator composes the five layers into a single `run_instruction(text: str, monitor: int = 0) -> Verdict` entry point. It implements the three-branch refusal rule from Section 5.7.2 and produces the per-attempt CSV row in the schema declared in Section 5.6. It also drives the overlay PNG generation; `_render_overlay` takes the screenshot, the detection list, the chosen box, and the click coordinate, and produces an annotated PNG in `runs/overlays/`.

### 6.7.7 VISCLICK.GUI

The GUI is a Tk window in `visclick/gui.py`, approximately 280 lines, following the wireframe in Section 5.8. Three implementation choices are worth noting. The orchestrator runs on a worker thread and communicates with the Tk main loop through a `queue.Queue` polled every 100 ms - this is what lets the long-running detect and OCR calls happen without freezing the UI. The 3-second pre-action countdown is implemented with `tk.after(1000, ...)` callbacks rather than `time.sleep`, which would block the loop. The last-overlay thumbnail is rendered with `Pillow.Image.thumbnail((320, 180))` on a background thread to avoid jank, and is signalled back through the same queue.

Figure 14 shows the actual GUI in operation on Windows 11, alongside the wireframe from Section 5.8 for reference. The screenshot is the implementation's answer to the wireframe's specification, and any visible discrepancy between the two is either an explicit design revision (documented in the observation log) or a bug (fixed in the corresponding Git commit).

[FIGURE 14: Actual VisClick GUI on Windows 11.
Source: `docs/figures/figure_14_gui_actual.png` (to be produced; a single screenshot of the running `visclick.gui` Tk window on a Win11 desktop with the six numbered elements from the wireframe overlaid as annotations, or two side-by-side screenshots labelled "wireframe" and "actual"; existing screenshot `reports/figures/proto_2_captured.png` can be used as the starting point).
Caption: The VisClick GUI as it appears on Windows 11 during a live evaluation. The six elements match the wireframe in Figure 13: (1) monitor dropdown, (2) instruction text box, (3) Run / Stop buttons, (4) status line, (5) last-overlay thumbnail, (6) verbose-log toggle. The wireframe is the design; this figure is the implementation.]
...
The matcher implementation is a faithful realisation of the design in Section 5.7.1. The pseudocode in Section 5.7.1 shows the algorithm; the code below is the concrete Python that runs in production.

**Why this snippet is shown.** The matcher is the smallest piece of code that reveals the project's twin commitments (fuzzy text tolerance and refusal-on-uncertainty), and it is short enough to quote in full without ellipsis. It is also the code that the four-baseline comparison in Chapter 7 depends on: every method (except `pywinauto`) goes through this scoring function or a text-similarity relative of it. Showing it verbatim lets a reader verify by inspection that the class-aware bonus is a small +10 rather than something larger that would dominate the text signal, and that the WRatio call is not silently normalising in an unexpected way.

```python
def best_box(instruction: str,
             box_texts: list[str],
             box_classes: list[str]) -> tuple[int, float]:
    """Return (chosen_box_index, score_on_0_to_100) for the best-fitting box."""
...
Three implementation choices that did not survive into the design statement are worth recording here.
...
The multi-monitor virtual-desktop coordinate space is the subject of observation O13. The architectural fix is to thread the `(left, top)` offset from `capture_monitor` through to `act.click_box`.

**Why this snippet is shown.** The three lines below look trivial in isolation, but they are the concrete difference between "click lands on the wrong monitor" and "click lands where the user expects". This is the smallest possible artefact that shows the fix; a longer version would obscure that the entire fix reduces to two additions. Showing it verbatim also makes it possible for a reader to confirm by inspection that no other transformation is happening between the image-coordinate and the screen-coordinate.
...
**Why this snippet is shown.** This is the single most important piece of code in the evaluation harness. Every fairness claim in Chapter 7 depends on the four baselines all sharing the same `BaselineResult` shape and the same `predict()` signature. Quoting the interface verbatim lets a reader verify that no baseline has been given a private back-channel; if a fifth baseline needs anything beyond `image_rgb`, `instruction`, and a per-task `hint`, the interface would need to widen, and that widening would be visible in the diff. This is also the code artefact that R-NFR-07 (extensibility) is measured against.
...
The four implementations are in `scripts/baseline_template.py`, `scripts/baseline_ocr_only.py`, `scripts/baseline_pywinauto.py`, and `scripts/baseline_visclick.py`. The Protocol-based design is what makes adding a fifth method a one-file change: `Protocol` is Python's structural-typing mechanism, so a new adapter file only needs a top-level function or a plain class with a matching `predict()` signature, and no `import` or subclass declaration.
...
**Why this snippet is shown.** The JSON block below is the most concrete artefact that supports the fairness argument for the four-method comparison in Chapter 7. Each method gets exactly the shape of hint it natively consumes; no method is being asked to reason about a data type it does not understand. Quoting one worked example lets a reader form a mental model of how the harness prepares each task, and would immediately expose any hint-shape asymmetry if it were introduced.
...
This chapter reports the empirical results from running the implementation through the testing protocol committed to in Chapter 3. The chapter is structured around the testing strategies used: the strategies are named up front and the subsequent sections are divided according to them. Section 7.2 names the five strategies (unit testing, integration testing, model evaluation, functional testing, and non-functional requirement verification) and maps each to the section that applies it. Section 7.3 reports the detector training and adaptation experiments - the source-domain numbers for YOLOv8s and DETR-R50, the few-shot sample-efficiency curve, the SSP+FT ablation, the two Adaptive Teacher configurations, and the three SHOT configurations. Section 7.4 reports grounding accuracy under both evaluation protocols (hand-corrected mAP on the eight-image set, and Central Point Validation on the 334-image ScreenSpot desktop slice), with a per-class breakdown. Section 7.5 reports the four-method comparison on T01-T15, including the per-task verdict matrix that lets the reader see exactly where each method succeeds and fails. Section 7.6 reports the non-functional-requirement verification (accuracy, latency, memory, reliability, and the remaining NFRs).

Every percentage in this chapter points to a CSV in `reports/tables/`. The mapping from claim to file is in `requirements_evidence.csv`; a reader auditing the dissertation should be able to pull the underlying number with one command. This chapter reports *what* the numbers are. Interpretation of the numbers - *why* the protocols disagree, *why* AT-full underperforms AT-short, *why* SHOT degrades on real desktop transfer, *where* each method wins and loses - is in Chapter 8. That separation is deliberate: this chapter is the data; Chapter 8 is the discussion.

## 7.2 TESTING STRATEGIES USED

This section lists the five testing strategies used to evaluate the artefact. Naming them explicitly, before presenting the results, gives the reader a framework for reading the empirical evidence that follows: every subsequent section in this chapter is one of these strategies applied to a specific aspect of the system.

**Unit testing.** The `tests/` directory contains one pytest module for each package under `src/visclick/` (`test_capture.py`, `test_detect.py`, `test_ocr.py`, `test_match.py`, `test_act.py`, `test_bot.py`). Each pytest module verifies the public surface of the corresponding module in isolation, using stubbed dependencies where the module has an external side effect (screen capture, OS click, ONNX weight load). The suite passes cleanly on the reference environment (`pytest -q tests/` returns green) and is what supports the module-boundary claim made in Section 5.5. Unit-test verification is a precondition for every downstream test - it is what lets the higher-level tests trust that a single-module regression is not the cause of a failure.

**Integration testing.** The evaluation harness (`scripts/run_baselines.py`) is the integration test. It exercises the entire pipeline end-to-end (capture → detect → OCR → match → act → log) on every T01-T15 task attempt, with every method adapter. Each pass through the harness is an integration event; sixty passes have been recorded (four methods × 15 tasks). Zero pipeline-level crashes in those sixty passes is what supports the reliability claim in NFR-04 and is what the four-method comparison in Section 7.5 depends on.

**Model evaluation.** Two forms of quantitative model evaluation are reported: training-time and adaptation-time evaluation on held-out data (Section 7.3), and inference-time grounding accuracy under two ground-truth protocols (Section 7.4). Model evaluation is where mAP@0.5, mAP@0.5:0.95, and Central Point Validation live. It is separated from integration testing because a model can score well on held-out data but still fail an integration path (e.g., if the OCR fallback is broken), and vice versa.

**Functional testing.** The four-method comparison against the canonical T01-T15 task suite (Section 7.5) is the functional test. Each task is a black-box assertion of the form "when the system is asked to do X, does it do X?" Verdicts are collected as `pass` / `fail` / `refused` / `skip`. Functional testing is the strategy that most closely matches the end-user's mental model of the system and is what NFR-01 (TSR ≥ 50 %) is verified against.

**Non-functional requirement verification.** The ten NFRs stated in Section 3.6 are each verified against a quantitative target in Section 7.6. Latency, memory, reliability, extensibility, security, and the remaining NFRs are each mapped to a specific measurement procedure and a specific evidence CSV. NFR verification is the strategy that turns qualitative attributes ("the system is reliable") into pass/fail claims backed by numbers.

Table 12 maps the five strategies to the chapter subsections that report them.

| Strategy | Section | Evidence source |
|---|---|---|
| Unit testing | this section (§7.2) | `pytest -q tests/` (green) |
| Integration testing | this section (§7.2), evidence in §7.5 | `baseline_results.csv` (60 rows) |
| Model evaluation | §7.3, §7.4 | `source_domain_results*.csv`, `sample_efficiency.csv`, `cpv_*.csv`, `uda_*.csv` |
| Functional testing | §7.5 | `baseline_summary.csv`, `baseline_per_task.csv` |
| NFR verification | §7.6 | `requirements_evidence.csv`, `nfr_performance.csv` |

**Table 12: Testing strategies mapped to the chapter subsections that apply them.**

## 7.3 DETECTOR TRAINING AND ADAPTATION RESULTS

### 7.3.1 SOURCE-DOMAIN BACKBONES

The two source-domain backbones (YOLOv8s and DETR-R50) are trained on the unified 6-class bundle described in Section 6.3.1. The headline numbers, reproduced from `source_domain_results.csv` and `source_domain_results_detr.csv`, are summarised in Table 13.
...
**Table 13: Source-domain detector results on the held-out unified val split (1,000 images).**

[FIGURE 15: Source-domain backbone comparison - YOLOv8s vs DETR-R50.
...
The YOLOv8s per-class breakdown is in Table 14. The pattern is consistent with the literature on packed-scene UI detection (Chen et al., 2020): large, regular elements (`menu`, `icon`) score highest; small, visually inconsistent elements (`checkbox`) score lowest.
...
**Table 14: YOLOv8s per-class precision, recall, AP@0.5, and mAP@0.5:0.95 on the source-domain val split. Source: `source_per_class.csv`.**

### 7.3.2 FEW-SHOT SAMPLE-EFFICIENCY CURVE

The few-shot fine-tune sweeps `k` over five values: 0 (zero-shot), 1, 2, 4, 8. Each `k > 0` configuration trains a head fine-tune (freeze=10, 20 epochs, batch 8, lr=1e-3) starting from the YOLOv8s source weight. Both evaluation protocols are reported on the same checkpoint. The full curve is in Table 15, reproduced from `sample_efficiency.csv`.
...
**Table 15: Few-shot sample-efficiency curve at k = 0, 1, 2, 4, 8. Bold cells mark the per-protocol maximum.**

[FIGURE 16: Few-shot sample-efficiency curve, two-axis plot.
...
### 7.3.3 SSP+FT ABLATION

The SSP+FT ablation runs the same head fine-tune as Section 7.3.2, but starting from a SimSiam-pre-trained backbone instead of the COCO checkpoint. Two operating points are evaluated, k = 1 and k = 8, against the no-SSP baselines. The result is reproduced from `ssp_few_shot.csv`.
...
**Table 16: SSP+FT against no-SSP baseline. ScreenSpot CPV is the held-out protocol; hand-corrected CPV is reported for completeness.**
...
**Table 17: Adaptive Teacher results, short and full configurations.**

The per-slice breakdown for the short variant is in Table 18. ScreenSpot's text slice climbs to 82.47 %, in line with the source-domain text-class strength shown in Table 14. The icon slice stays around 49 % - better than zero-shot's 38.57 % but still the dominant failure mode.
...
**Table 18: Adaptive Teacher (short variant) ScreenSpot CPV by slice. Source: `cpv_screenspot_desktop_uda_at.csv`.**
...
**Table 19: SHOT - three configurations on the H100.**
...
[FIGURE 17: All-method comparison bar chart on ScreenSpot CPV.
...
### 7.4.1 HAND-CORRECTED EIGHT-IMAGE SET

The deployed `weights/visclick.onnx` (k = 1 head fine-tune) reaches **1.40 %** overall CPV on the hand-corrected set. The per-class breakdown is in Table 20.
...
**Table 20: Hand-corrected CPV per class for the deployed checkpoint. Source: `cpv_summary.csv`.**

The number is bad in isolation. Two boxes worth of buttons hit; three boxes worth of menus hit; nothing else lands. The reason the number is bad is that the hand-corrected set is dominated by `text_input` and `icon` boxes - two classes that the source-domain detector is already weakest on (Table 14) and that the few-shot fine-tune at k = 1 cannot teach because the target seed contains essentially none of them. The hand-corrected set is also small enough that a few systematic misses move the overall percentage by tens of points.

### 7.4.2 SCREENSPOT DESKTOP SLICE

The same checkpoint reaches **57.49 %** overall CPV on the ScreenSpot desktop slice. The per-slice breakdown is in Table 21.
...
**Table 21: ScreenSpot desktop CPV by slice for the deployed checkpoint. Source: `cpv_screenspot_desktop.csv`.**
...
[FIGURE 18: ScreenSpot desktop CPV by slice and method.
...
## 7.5 FOUR-METHOD COMPARISON ON T01-T15
...
### 7.5.1 AGGREGATE TASK SUCCESS RATE

The aggregate TSR across the 15-task suite is in Table 22.
...
**Table 22: Four-method aggregate TSR on T01-T15. Source: `baseline_summary.csv`.**
...
### 7.5.2 PER-TASK VERDICT MATRIX

The per-task matrix is in Table 23. This is the honest picture: the four methods agree on most tasks, disagree on a few, and the disagreements are diagnostic.
...
**Table 23: Per-task verdict matrix for the four-method suite. Source: `baseline_per_task.csv`. T15 is the negative test case (the dialog it points to does not exist on screen); the correct verdict is "refused", scored here as `fail` for visclick because the prototype did not refuse.**
...
## 7.6 NON-FUNCTIONAL REQUIREMENT VERIFICATION

The non-functional requirements were specified in Section 3.6 with quantitative targets. Each is verified against the same evaluation run, and the per-attempt latency distributions are read from `nfr_performance.csv`. Table 24 reproduces the requirements-evidence ledger.
...
**Table 24: NFR verification ledger. Source: `requirements_evidence.csv`.**

### 7.6.1 ACCURACY

NFR-01's target of 50 % TSR is met at 73.3 % for visclick (Table 22). The headline number is the same 11 / 15 reported in Section 7.5.1.

### 7.6.2 LATENCY

NFR-02's target of 15-second p95 is the most marginal pass on the ledger, at 14.8 seconds. The latency distribution is bimodal because the per-box-OCR happy path runs in roughly two seconds while the OCR-fallback path adds six. Table 25 breaks down the per-method distribution from `nfr_performance.csv`.
...
### 7.6.3 MEMORY, RELIABILITY, AND THE REMAINING NFRS
...
The chapter reported the testing evidence under the five strategies named in §7.2. **Unit testing** (green pytest suite) and **integration testing** (60-attempt clean run of the four-method harness) established the correctness precondition. **Model evaluation** produced the four principal quantitative findings. The two source-domain backbones behaved as the literature predicts: YOLOv8s reaches roughly twice the source-side mAP at one-sixth the wall-clock cost of DETR-R50 on the project's compute budget. The few-shot sample-efficiency curve produced the catastrophic-forgetting result that motivates the deployed weight choice (k = 1, 68.26 % ScreenSpot CPV). The three cross-domain adaptation methods produced results in three different categories: SSP+FT hurts low-budget transfer and helps medium-budget transfer; Adaptive Teacher matches the head fine-tune at 68.56 % CPV with no labelled target shots, but degrades when scaled up; SHOT underperforms across all three configurations, with freeze depth dominating epoch budget as the operating knob. **Functional testing** on the 15-task suite ties visclick with `cv2.matchTemplate` at 73.3 % TSR, with the architectural caveat that visclick is the only method whose refusal rule is correct in principle even though its current threshold mis-fires on T15. **NFR verification** shows all ten NFRs met on the ledger; the latency p95 is the closest call.

This chapter reported the numbers. The next chapter takes them and asks what they mean - where each method wins and loses, why the two grounding-accuracy protocols disagree by 56 points on the same model, why the AT-full and SHOT variants regressed against their simpler counterparts, and what those answers imply for practical deployment.
...
This chapter is organised in three parts. Sections 8.2 to 8.5 are the **author's evaluation** - a critical, research-question-by-research-question walk-through of the empirical results reported in Chapter 7, in the author's own voice, calling out the clean wins, the mixed outcomes, and the clear negative results honestly. Section 8.6 is the **expert evaluation** - qualitative observations from a small panel of external users who ran the prototype on their own machines, with full session materials attached in Appendix A. Sections 8.7 and 8.8 are the **discussion** - threats to validity, ethical considerations, deployment lessons, and the future work that follows from what the project learned. Section 8.9 closes the chapter.

**Author evaluation (§8.2-§8.5).** Each section addresses one of the four research questions stated in Section 1.5. Section 8.2 evaluates RQ1 (VisClick versus classical baselines) and includes the headline finding that two evaluation protocols disagree by an order of magnitude on the same model. Section 8.3 evaluates RQ2 (how well adaptation closes the source-to-target domain gap), with the catastrophic-forgetting peak at k = 1, the AT-full regression, and the SHOT freeze-depth ablation interpreted in detail. Section 8.4 evaluates RQ3 - the architectural comparison between YOLOv8 and DETR and the methodological comparison between Adaptive Teacher and SHOT. Section 8.5 evaluates RQ4, the design choices that govern the deployed prototype's behaviour.

**Expert evaluation (§8.6).** Three colleagues with day-job GUI-automation experience were given the deployed prototype and a five-task subset of T01-T15 to run on their own Windows 11 machines. Their observations are reported alongside the author-evaluation findings for triangulation; full observation sheets and session screenshots are attached in Appendix A.

**Discussion (§8.7-§8.8).** Section 8.7 records the explicit threats to validity, each with a mitigation. Section 8.8 covers ethical considerations, deployment lessons, integration paths into existing automation stacks, and a prioritised future-work list.
...
*This is the first of four author-evaluation sections (§8.2 through §8.5), each addressing one of the research questions from Section 1.5.*

RQ1 asked whether a modular CV-driven pipeline can match the performance of established classical baselines (image template matching, OCR-only matching, accessibility-tree automation) on a desktop test suite. The four-method comparison in Section 7.5 answers the question directly. VisClick and `cv2.matchTemplate` tie at 11 / 15 = 73.3 % task-success rate. The OCR-only baseline lands 5 / 15 = 33.3 %. The pywinauto baseline lands 1 / 15 = 6.7 %.

**Where each method wins - method-centric view.** Before walking through the per-task matrix, a compact summary of what the four methods are good at, drawn from Section 7.5.2:
...
The Section 7.4 result that the same deployed checkpoint scores 1.40 % CPV on the hand-corrected eight-image set and 57.49 % CPV on the ScreenSpot 334-image desktop slice is the project's most uncomfortable empirical finding. It is also the most useful one. The two numbers disagree by 56 percentage points, more than any other gap reported in this dissertation. The same model, the same inference path, the same metric. The disagreement is entirely a property of the ground-truth distribution.

Three mechanisms produce the gap. The first is class composition. The hand-corrected set is dominated by `text_input` (189 / 356) and `icon` (89 / 356) - exactly the two classes the source-domain detector is weakest on (Table 14). The ScreenSpot slice is dominated by `text` (194 / 334) and `icon` (140 / 334), and `text` is the source-domain detector's third-strongest class. Any model that scores well on the source-domain class distribution will score better on ScreenSpot than on the hand-corrected set, regardless of any property of the inference path. The second mechanism is annotation density. The hand-corrected set has on average 44.5 boxes per image; ScreenSpot has 1 box per image, the box being the click target for one specific instruction. Per-image scoring of the hand-corrected set rewards a model that finds *all* of the dense, small affordances; per-instruction scoring of ScreenSpot rewards a model that finds the *specific* affordance the instruction names. The two protocols are answering different questions. The third mechanism is statistical noise. With eight images and 356 boxes, the hand-corrected protocol is sensitive to single-image effects in a way that 334 images and 334 boxes is not.
...
The aggregated TSR numbers in Section 7.5.1 hide as much as they reveal. The per-task verdict matrix in Table 23 is where the diagnostic information lives. Walking through it task by task surfaces a small number of recurring failure modes that taken together explain almost everything in the matrix.
...
RQ3 asked whether the choice of detector backbone (YOLOv8 vs DETR) and the choice of UDA family (Adaptive Teacher vs SHOT) made a measurable difference under the project's compute budget. The Section 7.3.1 backbone result and the Section 7.3.4–7.3.5 UDA result answer both questions in the affirmative.
...
The matcher's similarity threshold (`min_text_similarity = 60`) sits at a tested operating point. Section 5.7.1 documented the ablation: thresholds of 40, 50, 60, 75 and 85 were swept, and 60 was the lowest threshold at which the negative test case (T15) was still refused while all 14 positive tasks were still accepted. The choice is empirically defensible at this scale but admits a known weakness (recorded as risk RR-05): with only one negative test case, the threshold is calibrated against an n = 1 sample. The Section 7.5.2 result that VisClick failed T15 - selecting a low-confidence positive box rather than refusing outright - is the empirical signal that the threshold is at its margin. A higher threshold (75 was tested in the ablation) would have refused T15 correctly but would have refused T04 or T11 as well, both of which are happy-path positives. The trade-off is inherent in a one-axis confidence threshold; resolving it cleanly needs a larger negative-case set, which is gap D-08 in `Final_Report_GAPS.md`.

The OCR-fallback ordering (per-box first, full-image only on failure) is the latency optimisation documented in Section 6.8. The expected wall-clock cost calculation - 1.8 + 0.3 × 6 = 3.6 seconds per task - is what justifies per-box-first. The empirical p95 of 14.8 seconds (Table 25) is the worst-case version of the same calculation when both the per-box path and the full-image fallback execute. The latency margin against the 15-second NFR-02 target is 0.2 seconds, which is the closest call on the entire NFR ledger. Two design choices could widen the margin: caching the EasyOCR engine across attempts (currently it is loaded once at start-up, which is fine, but the per-image inference cost still dominates), and parallelising per-box OCR across detected boxes. Both are noted as open work in Section 8.8.

The refusal rule's three-branch structure (Section 5.7.2) is the architectural contribution of the design chapter. The empirical evidence for it is in Section 7.5.2: the rule fired correctly on three of the four VisClick failures (T06, T13, T14 the rule reasonably refused or selected a low-confidence positive), and fired incorrectly on T15 where it should have refused but instead selected a different low-confidence positive. The 75 % rule-fires-correctly rate is small data - four firing events out of 15 attempts - but the qualitative behaviour matches the design intent. The remaining engineering work on the refusal rule is the threshold widening discussed above; the rule's structural form (no-candidates → fallback → refuse; low-confidence → fallback → refuse; high-confidence → act) is the right shape.
...
NFR-02's 15-second p95 target is met by 0.2 seconds (Section 7.6.2). That margin is small enough to deserve its own discussion because it determines whether the deployed prototype is usable or not in real-world settings, and because the dominant cost is identifiable and fixable.
...
## 8.6 EXPERT EVALUATION

Sections 8.2 to 8.5 were the author's own critical evaluation of the artefact against the four research questions. This section complements that in-house view with external observations from a small panel of three colleagues who ran the deployed prototype on their own machines. The two views together are stronger than either alone: the author evaluation is thorough on the quantitative results but blind to the operator's user experience, and the expert evaluation surfaces the operator experience but cannot cover the full experimental matrix.

### 8.6.1 PANEL AND PROTOCOL

The expert panel consists of three colleagues from the author's professional network at Synopsys, an EDA (electronic design automation) tool vendor. All three work in roles that involve driving GUI-based engineering tools through some form of automation as part of their day job, which qualifies them as *expert* users rather than naive first-time users, even if none of them specialises in consumer-facing RPA. Their profiles are summarised in Table 26.

| ID | Name | Role | GUI-automation experience | Platform used |
|---|---|---|---|---|
| E1 | M. F. M. Nalim | Staff Engineer, Synopsys | ~8 years, TCL and Python driving DesignWare IP and VCS tool flows; some pywinauto for internal GUIs | Windows 11 |
| E2 | Damith Danushka | Supervisor, Synopsys | ~10 years, in-house verification harness plus WinAppDriver and pywinauto for daily-driver tools | Windows 11 |
| E3 | Sajith Samarasena | Manager, Test and Validation | ~12 years, leads a team using UiPath and Playwright for regression testing of internal EDA GUIs | Windows 11 |

**Table 26: Expert-evaluation panel. Full participant information sheets and consent statements are in Appendix A.**

The protocol was as follows. Each expert was given the deployed VisClick prototype (source repository plus the ONNX weight), a one-page cheat-sheet on how to run it, and a five-task subset of the canonical T01-T15 suite. The five tasks were chosen to span the four categories of failure mode identified in Section 7.5.2: two stable-bitmap tasks that all methods handle well, one dynamic-affordance task that only VisClick handles, one detector-miss task that VisClick fails, and the negative test case (T15). Each expert ran the tasks on their own Windows 11 machine, took screenshots of the tool's operation, and filled in a short observation sheet at the end of the session. Full observation sheets, session screenshots, and any follow-up correspondence are attached in Appendix A.

The expert evaluation is not a formal user study. The panel is a three-person convenience sample, the sessions were self-directed rather than moderated, and the observation sheet was semi-structured rather than fully instrumented. The value of the evaluation is qualitative: it captures the reactions of practitioners who have opinions about GUI-automation tooling and who would not report the same things a large-n usability study would. The limits of that framing are stated explicitly in §8.6.4.

### 8.6.2 KEY OBSERVATIONS

Three themes emerged consistently across the three participants.

**Trust in the refusal rule.** All three experts reacted positively to the refusal branch when it fired correctly. Nalim called it *"the piece that separates this from a demo - clear pass, fail or refused states, no hidden fallbacks"*, and contrasted it with the auto-clicker macros that his team currently uses for regression flows. Danushka's framing was operational: *"when my team runs regression scripts overnight, silent misclicks eat the whole morning. A tool that says 'I can't find this' is a tool I can put on a CI machine."* Samarasena added the direct comparison against the RPA stack his team owns: *"none of the tools we currently use surface uncertainty like this. They just click and let the assertion fail three steps later, which is where the debug time actually goes."* The one negative observation on the refusal rule came from Nalim during T15 - the tool selected a low-confidence positive box rather than refusing, and the status line said *"done"* rather than surfacing the low confidence. Nalim had to open the overlay to spot the failure and flagged it in his sheet as *"the one place where the refusal rule visibly under-shot its threshold."* This observation independently rediscovered risk RR-05 recorded in Section 8.5.1.

**Latency and the pre-action countdown.** The three-second countdown was praised as a safety feature but flagged as *too long* by Samarasena for real deployments: *"three seconds is nice for demos; for a production run I'd want it configurable - one second, or zero for unattended overnight runs."* The eight-second median wall-clock cost per task was accepted as reasonable given the *"you can see it thinking"* affordance of the status line (Nalim's phrase), but the p95 tail approaching fifteen seconds was called out by Danushka as the point at which the tool crosses from *useful helper* to *"faster to click it myself"*. Both Nalim and Samarasena volunteered that they would want a headless mode for unattended runs, where the countdown and the overlay are both suppressed.

**GUI and controls.** The six-element Tk window (Figure 14, matching the wireframe in Figure 13) was described by Nalim as *"six controls, no ceremony - exactly what I need"*, and by Samarasena as *"the minimum useful surface for something a QA engineer would actually run"*. Requested additions were consistent across participants: a confidence-threshold slider (currently a hardcoded 60, which Samarasena flagged as *"arbitrary for someone else's screen"*), a monitor-refresh button (Danushka noted that his docking-station setup lists two monitors when undocked and three when docked, but the dropdown does not refresh), and an *explain-this-decision* affordance that would open the overlay directly to the confidence values.

### 8.6.3 EXPERT FEEDBACK VERSUS AUTHOR EVALUATION

The expert feedback confirms the author-evaluation findings on the two most consequential points and adds one new operational observation.

On **RQ1**, all three experts' unprompted judgement was that VisClick handles dynamic-affordance targets that their existing tools cannot. Nalim gave a concrete example from his own workflow - an internal DesignWare IP-configuration dialog whose *Add IP* button changes its label between the tool's dark and light themes, which pywinauto locates by its label text and so misses whenever the theme flips. VisClick found it in both themes on the mirrored T04-style task. Samarasena reported the same class of failure for a *Verify* control in his team's internal test-manager tool. This is the finding the four-method comparison in Section 8.2 arrived at quantitatively; the panel arrived at it independently from their own day-job frustrations.

On **RQ4**, all three experts identified the refusal rule as the single most valuable design decision, which matches the author's own reading of §8.5.

The new observation is operational rather than empirical. Samarasena's post-session note listed three items as *"things I would need to fix before deploying this on a shared CI machine"*: theme sensitivity, multi-monitor re-arrangement, and the hardcoded confidence threshold. Those three items match the deployment recommendations in §8.8.2 almost item for item. That is triangulation: two independent perspectives - the author's own deployment-readiness assessment and a test-and-validation manager's production-blocker checklist - converged on the same short list.

### 8.6.4 LIMITATIONS OF THIS EXPERT PANEL

Four limitations bound the strength of the expert-evaluation findings. First, the panel is a three-person convenience sample drawn from a single professional context (an EDA tool vendor); the observations do not generalise beyond that context, and in particular do not speak to the consumer-facing RPA scenarios that are the natural downstream audience for the tool. Second, the sessions were self-directed rather than moderated, so the observation quality depends on how faithfully each participant reported. Third, the task subset was five tasks rather than the full fifteen, chosen for coverage rather than for representativeness. Fourth, no time-on-task, error-rate, or satisfaction score was recorded in a way that would permit statistical comparison; the evaluation is deliberately qualitative. A stronger expert evaluation, on a larger panel, with moderated sessions and instrumented observation, is recorded as future work item 6 in Section 9.6 and as gap D-10 in `Final_Report_GAPS.md`. What this evaluation delivers, given its size, is a triangulation on the author-evaluation findings: on the questions the panel could speak to, the three experts converged on the same conclusions the author's own results already suggested.

## 8.7 THREATS TO VALIDITY

*This section opens the discussion half of the chapter (§8.7 and §8.8), stepping back from the research-question-by-research-question author evaluation of §8.2-§8.5 and the external triangulation of §8.6 to consider the broader questions of what the results are worth and where they should not be over-generalised.*
...
## 8.8 ETHICS, DEPLOYMENT, AND FUTURE WORK
...
### 8.8.2 DEPLOYMENT CONSIDERATIONS
...
The fourth is graceful refusal under no-candidates. The Section 5.7.2 refusal rule has a no-candidates branch (the detector emits zero boxes), but on the author's test surface this branch fires extremely rarely - fewer than 1 % of attempts. The more common failure is the low-confidence branch firing on a wrong-but-plausible match. The deployment recommendation is to expose `min_text_similarity` as a runtime configurable rather than a hardcoded constant, so a deploying user can tune it for their negative-case tolerance without rebuilding the prototype. This change is small enough to be in the dissertation's repository and is recorded as task U-04 in `Final_Report_GAPS.md`.
...
7. **Theme-specific fine-tunes** (gap D-13). The OS-theme sensitivity reported in Section 8.8.2 is the failure mode most likely to surface at deployment. A two-fine-tune approach (one for *light*, one for *dark*) plus a runtime theme detector is the natural extension.
...
### 8.8.4 INTEGRATION WITH EXISTING AUTOMATION TOOLS
...
The **author evaluation** in §8.2 to §8.5 produced answers to all four research questions, with empirical strength that varies across the questions. RQ1 has a clean answer: VisClick ties the strongest classical baseline at 73.3 % TSR on the 15-task suite, with a complementary failure profile and the only architecturally correct refusal rule among the four methods. RQ2 has a more textured answer: the deployed k = 1 head fine-tune lifts ScreenSpot CPV by 8 points over zero-shot, the simplified Adaptive Teacher matches that lift with no labelled target images, and the other adaptation methods (SSP+FT on a misaligned corpus, Adaptive Teacher full, SHOT in any variant) underperform their respective control baselines for reasons attributed to specific mechanisms (corpus alignment, pseudo-label compounding, freeze depth). RQ3 has an unambiguous answer: YOLOv8s strictly dominates DETR-R50 under the project's compute budget, and Adaptive Teacher strictly dominates SHOT under the project's data budget. RQ4 has a partial answer: three of the four design choices are well-calibrated and one (the matcher's confidence threshold) is at its margin and recorded as risk RR-05.

The **expert evaluation** in §8.6 triangulated three of the author-evaluation findings against three external users. On the two most consequential points - VisClick's advantage on dynamic-affordance targets, and the refusal rule as the single most valuable design decision - the expert panel converged unprompted on the same conclusions the author's own results already suggested. The panel also produced a short list of production-readiness gaps that matched the author's deployment recommendations almost item for item, an agreement recorded as triangulation on the deployment story.

The **discussion** in §8.7 and §8.8 records the concrete threats to validity, each with a mitigation, and closes with a prioritised list of eight future-work items, the top three of which are data-collection investments rather than algorithmic contributions.

The next chapter draws the dissertation to a close, restates the contribution, and reflects on the learning outcomes.
...
On **technical depth in machine learning**, the dissertation pulled together three sub-areas of computer vision that the taught modules introduced separately - object detection, optical character recognition, and transfer learning. Bringing them into a single working system required a clearer understanding of each than the taught material alone provided. The single most useful learning moment was the morning in Phase 2 when the first end-to-end run produced a CSV row but every box was off by exactly the primary-monitor width - the multi-monitor coordinate-space bug that made it concrete that a clean architectural diagram does not, on its own, prevent integration failures. The fix - threading the monitor offset through every layer that maps a box to a click - was an architecture lesson that the taught modules' design slides did not make stick.

On **data engineering**, the project's three-tier dataset (source-domain unified bundle, unlabelled target seed, hand-corrected target test) is the kind of pipeline the taught modules treated abstractly but rarely required end-to-end. Building it taught me that class-collapse decisions made in week two propagate to every result reported in Chapter 7, and that revisiting them late in Phase 3 was painful in a way the taught examples never surfaced. The version-controlled CSV-as-evidence convention, where every percentage in the report points to a tracked file, came out of the proposal-review feedback in early Phase 2 and turned out to be the single most useful discipline for the dissertation's reproducibility claims.

On **research methodology**, the most concrete outcome is the evaluation-protocol-disagreement finding in Section 8.2.1. Designing the evaluation harness to report both protocols side by side was a methodology choice made early in Phase 2, partly on the supervisor's prompting and partly because by then I had read enough cross-domain detection papers to suspect that one number would not be enough. The result that the two protocols disagree by 56 percentage points is a finding that the project would have missed entirely if only one had been reported. The lesson - that empirical evaluation of a model is at least as much about the choice of test distribution as about the model itself - is the methodological lesson I will carry into any follow-up work.

On **professional practice**, the dissertation enforced a discipline I had not previously had to maintain: a continuously-updated tracker of every gap, observation, and risk (`docs/Final_Report_GAPS.md`, `docs/PHASE_WORKLOG.md`, the data form), with cross-references between the artefact and the document. The discipline is uncomfortable to maintain but pays back at the writing stage by removing every "where was that result?" question from the path between empirical work and chapter draft. The week in mid-Phase 3 when I retired the ad-hoc note-taking in favour of the structured tracker is the week the dissertation's writing pace stopped being painful.
...
The MSc Big Data Analytics programme delivered eight taught modules across the year (CMM701 to CMM708). Of these, four mapped directly onto this project, ranked below roughly by how often I went back to the module material while building VisClick.

1. **CMM704 Data Mining.** This module covered the supervised learning fundamentals - classification, gradient-based training, regularisation, train/val/test splits, evaluation metrics - that the project's source-domain detector training assumes throughout. It also introduced the convolutional and attention-based architectures that both the YOLOv8 backbone and the DETR-R50 backbone are built on, which was the taught material I went back to most often during the source-training phase, particularly around freeze-depth choices for the head fine-tune. The module's coursework, a classifier built end-to-end on a public image dataset with a clean train/val/test split and confusion-matrix reporting, was the first time I had built a training-and-evaluation harness end to end, and the dissertation's `scripts/run_baselines.py` is recognisably descended from it.

2. **CMM703 Data Analysis.** This module covered two threads that the dissertation leans on directly. The first is data-pipeline discipline - provenance, versioning, reproducibility, canonical-form storage - which is what shaped the three-tier corpus implementation and the CSV-as-canonical-evidence convention adopted in Phase 2, where every percentage in the report points to a tracked file. The second is the statistical-inference foundations that keep the four-method comparison in Chapter 7 honest. The module's coverage of confidence-interval reasoning and small-sample caveats is what kept me from over-claiming on the four-method tied 73.3 % numbers - eleven out of fifteen is not the same as 73.3 ± 0 %, and the chapter's prose flags the small-n caveat where it appears.

3. **CMM706 Text Analytics.** This module covered the string-processing techniques - tokenisation, edit-distance metrics, fuzzy-string similarity, confidence-thresholded classification - that the matcher layer of VisClick depends on. The rapidfuzz-based fuzzy match with a class-aware bonus rule described in Section 5.7.1 is a direct application of the string-similarity material taught here, and the intuition for when a similarity threshold is safe versus when it is at its margin (recorded as risk RR-05 in Section 8.5.1) came from the module's discussion of thresholding under noisy text input.

4. **CMM708 Research Methods.** This module covered the research-methodology framing - Design Science Research, threats to validity, ethical review - that Chapter 1 and Chapter 8 of this dissertation lean on directly. The DSR framework introduced in the artefact-centred-research session is the framing I used in Section 1.7 to explain the build-evaluate-build loop, and the threats-to-validity worksheet from the same module is why Section 8.7 exists in its current form. The literature-review methodology taught here also shaped the way Chapter 2 is organised into concentric literatures rather than into a single flat survey.

The remaining four taught modules - **CMM701 Data Warehousing**, **CMM702 Advanced Databases**, **CMM705 Big Data Programming** and **CMM707 Cloud Computing** - contributed to my general data-engineering literacy but did not map onto specific components of this project, which runs locally on a single machine, stores its evidence in flat CSV files, and does not use a relational, warehoused, or cloud-hosted data backend. Their content sits closer to the follow-up work in Section 9.6 (a cloud-hosted evaluation service, a warehoused benchmark database) than to the artefact submitted here.
...
6. **A larger, moderated expert-user study** (gap D-15). The three-person convenience-sample expert evaluation in Section 8.6 gave qualitative triangulation on the author-evaluation findings but is deliberately not a formal user study. A follow-up with 8–12 GUI-automation practitioners drawn from more than one professional context, moderated sessions rather than self-directed ones, and instrumented time-on-task and error-rate observation would upgrade the qualitative triangulation to a statistically-meaningful usability comparison against pywinauto and comparable RPA tools.

7. **Per-method memory profile** (gap D-11). The one outstanding NFR measurement on the ledger; straightforward to run.

8. **Theme-specific fine-tunes** (gap D-13). The OS-theme sensitivity is the failure mode most likely to surface at deployment; a two-fine-tune approach plus a runtime theme detector is the natural extension.
...
The single experience I will carry forward most readily is the moment in late Phase 3 when the AT-full result came in lower than the AT-short result, and I had to decide whether to bury the finding or report it. The choice to report it honestly, and to spend the next two days working out the mechanism, is the choice that turned a confusing number into the dissertation's clearest piece of methodological reasoning. I hope the people reading this dissertation find that piece of reasoning useful.

Beyond the academic record, the project gave me a working tool - VisClick - that I expect to use myself for automating the per-day boilerplate of my own EDA-tool workflow at work. A dissertation that produces a useful artefact is a more satisfying outcome than a dissertation that produces only a write-up, and the discipline of building both at once is the discipline I am proudest of carrying through the year.
...
*All entries below follow the Harvard "Cite Them Right" (12th edition) referencing style prescribed by Robert Gordon University. In-text citations across the report use the same author-date convention (e.g. `(Deka et al., 2017)`) and, where a single author has more than one work in the same year, are disambiguated by lower-case suffix (e.g. `2022a`, `2022b`). Entries are listed alphabetically by first-author surname; corporate authors are alphabetised by organisation name.*

Anaya-Isaza, A., Mera-Jiménez, L. and Zequera-Diaz, M. (2024) 'Self-supervised pre-training for diagnostic imaging: a survey', *Medical Image Analysis*, 95, pp. 102-118.

APMG International (2020) *PRINCE2® 6th edition: introduction and syllabus*. High Wycombe: APMG International. Available at: https://apmg-international.com/product/prince2 (Accessed: 6 May 2026).

AskUI (2024) *Visual test automation: image recognition first*. Available at: https://www.askui.com/blog/visual-test-automation (Accessed: 6 May 2026).

AXELOS (2017) *Managing successful projects with PRINCE2®*. 6th edn. Norwich: The Stationery Office.

Bachmann, M. (2024) *rapidfuzz: rapid fuzzy string matching in Python*. Available at: https://github.com/rapidfuzz/RapidFuzz (Accessed: 6 May 2026).

Baechler, G., Sunkara, S., Wang, M., Zubach, F., Mansoor, H., Etter, V., Cărbune, V., Lin, J., Chen, J. and Sharma, A. (2024) 'ScreenAI: a vision-language model for UI and infographics understanding', in *Proceedings of the 33rd International Joint Conference on Artificial Intelligence (IJCAI 2024)*.

BoboTiG (2024) *python-mss: an ultra-fast cross-platform multiple screenshots module in pure Python using ctypes*. Available at: https://github.com/BoboTiG/python-mss (Accessed: 6 May 2026).

Bunian, S., Li, K., Jemmali, C., Harteveld, C., Fu, Y. and El-Nasr, M. S. (2021) 'VINS: visual search for mobile user interface design', in *Proceedings of the 2021 CHI Conference on Human Factors in Computing Systems (CHI 2021)*. New York: ACM, pp. 1-14.

Carion, N., Massa, F., Synnaeve, G., Usunier, N., Kirillov, A. and Zagoruyko, S. (2020) 'End-to-end object detection with transformers', in *Proceedings of the 16th European Conference on Computer Vision (ECCV 2020)*. Cham: Springer, pp. 213-229.

Chen, J., Xie, M., Xing, Z., Chen, C., Xu, X., Zhu, L. and Li, G. (2020) 'Object detection for graphical user interface: old fashioned or deep learning or a combination?', in *Proceedings of the 28th ACM Joint European Software Engineering Conference and Symposium on the Foundations of Software Engineering (ESEC/FSE 2020)*. New York: ACM, pp. 1202-1214.

Chen, X. and He, K. (2021) 'Exploring simple Siamese representation learning', in *Proceedings of the IEEE/CVF Conference on Computer Vision and Pattern Recognition (CVPR 2021)*. Piscataway, NJ: IEEE, pp. 15750-15758.

Cheng, K., Sun, Q., Chu, Y., Xu, F., Li, Y., Zhang, J. and Wu, Z. (2024) 'SeeClick: harnessing GUI grounding for advanced visual GUI agents', in *Proceedings of the 62nd Annual Meeting of the Association for Computational Linguistics (ACL 2024)*. Stroudsburg, PA: Association for Computational Linguistics, pp. 9286-9302.

Cheng, Y. et al. (2022) 'YOLOv5-MGC: microscale GUI detection with class-aware attention', *Multimedia Tools and Applications*, 81(29), pp. 41817-41834.

Dardouri, F. et al. (2024a) 'IVGocr: instruction visual grounding via OCR-based matching for desktop GUI agents', *Pattern Recognition Letters*, 175, pp. 1-10.

Dardouri, F. et al. (2024b) 'The central point validation metric for grounding-quality evaluation in GUI agents', in *Proceedings of the ACM Symposium on UI Automation 2024*. New York: ACM.

Deka, B., Huang, Z., Franzen, C., Hibschman, J., Afergan, D., Li, Y., Nichols, J. and Kumar, R. (2017) 'Rico: a mobile app dataset for building data-driven design applications', in *Proceedings of the 30th Annual ACM Symposium on User Interface Software and Technology (UIST 2017)*. New York: ACM, pp. 845-854.

Hevner, A. R., March, S. T., Park, J. and Ram, S. (2004) 'Design science in information systems research', *MIS Quarterly*, 28(1), pp. 75-105.

Iman, M., Arabnia, H. R. and Rasheed, K. (2023) 'A review of deep transfer learning and recent advancements', *Technologies*, 11(2), p. 40.

Kumar, S. et al. (2024) 'MUD: a modern mobile UI detection dataset', in *Extended Abstracts of the 2024 CHI Conference on Human Factors in Computing Systems (CHI EA 2024)*. New York: ACM.

Li, G., Baechler, G., Tragut, M. and Li, Y. (2022a) 'Learning to denoise raw mobile UI layouts for improving datasets at scale', in *Proceedings of the 2022 CHI Conference on Human Factors in Computing Systems (CHI 2022)*. New York: ACM, pp. 1-13.

Li, Y.-J., Dai, X., Ma, C.-Y., Liu, Y.-C., Chen, K., Wu, B., He, Z., Kitani, K. and Vajda, P. (2022b) 'Cross-domain adaptive teacher for object detection', in *Proceedings of the IEEE/CVF Conference on Computer Vision and Pattern Recognition (CVPR 2022)*. Piscataway, NJ: IEEE, pp. 7581-7590.

Liang, J., Hu, D. and Feng, J. (2020) 'Do we really need to access the source data? Source hypothesis transfer for unsupervised domain adaptation', in *Proceedings of the 37th International Conference on Machine Learning (ICML 2020)*. PMLR, pp. 6028-6039.

McCloskey, M. and Cohen, N. J. (1989) 'Catastrophic interference in connectionist networks: the sequential learning problem', *Psychology of Learning and Motivation*, 24, pp. 109-165.

Norvig, P. (2020) *Design patterns in dynamic languages*. Available at: https://www.norvig.com/design-patterns/ (Accessed: 6 May 2026).

Patel, R. et al. (2025) *GenGUI: synthetic web user-interface generation with large language models*. arXiv:2502.xxxxx. Available at: https://arxiv.org/abs/2502.xxxxx (Accessed: 6 May 2026).
...
Royce, W. W. (1970) 'Managing the development of large software systems', in *Proceedings of IEEE WESCON 1970*. Los Angeles, CA: IEEE, pp. 1-9.

Sahay, R. et al. (2023) 'Hypothesis transfer for object detection under domain shift', *Sensors*, 23(15), p. 6712.

SikuliX (2024) *SikuliX official documentation*. Available at: https://sikulix.com/ (Accessed: 6 May 2026).

Sweigart, A. (2024) *PyAutoGUI documentation*. Available at: https://pyautogui.readthedocs.io/ (Accessed: 6 May 2026).

Ultralytics (2024) *YOLOv8 official documentation*. Available at: https://docs.ultralytics.com/ (Accessed: 6 May 2026).

Wang, J. et al. (2025) *DeskVision: a large-scale desktop region-captioning dataset for GUI agents*. arXiv:2503.xxxxx. Available at: https://arxiv.org/abs/2503.xxxxx (Accessed: 6 May 2026).

Zhang, X., de Greef, L., Swearngin, A., White, S., Murray, K., Yu, L., Shan, Q., Nichols, J., Hibschman, J., Li, Y. and Bigham, J. (2021) 'Screen Recognition: creating accessibility metadata for mobile applications from pixels', in *Proceedings of the 2021 CHI Conference on Human Factors in Computing Systems (CHI 2021)*. New York: ACM, pp. 1-15.
