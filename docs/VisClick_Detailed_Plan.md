...
- [x] **1.C.1** **`scripts/baseline_template.py` — SikuliX-style template matching.** Captures reference PNGs of Windows 11 / Chrome / VS Code controls (Save, Cancel, OK, search icon, address bar) into `samples/templates/`. Implementation does `cv2.matchTemplate(screenshot, template, TM_CCOEFF_NORMED)` at 3 scales (0.85 / 1.0 / 1.15), picks the best match if score > 0.7, clicks. **DONE (scaffolding) May 6, 2026 — script written, capture instructions in `samples/templates/README.md`, runs on Windows.** Pending: user captures reference PNGs and runs the runner.
- [x] **1.C.2** **`scripts/baseline_ocr_only.py` — OCR-only.** No detection model at all. Runs EasyOCR (or Tesseract) on the full screenshot via the existing `visclick.ocr.text_ground` helper (the same helper VisClick itself uses as fallback), finds the best fuzzy-match word, clicks its bbox centre. Effectively "VisClick without the YOLO step". **DONE (scaffolding) May 6, 2026 — script written, no fresh deps required.** Pending: user runs the runner.
- [x] **1.C.3** **`scripts/baseline_pywinauto.py` — accessibility-tree.** Uses `pywinauto.Desktop(backend='uia').descendants(name='Save', control_type='Button')` to locate controls via the Windows UI Automation API. Walks every visible top-level window, returns the first matching descendant's `rectangle()`. Demonstrates *"if accessibility worked, ML would be unnecessary — but it doesn't on Electron / partial UIA apps"*. **DONE (scaffolding) May 6, 2026 — script written; `pywinauto` added as optional `[windows]` extra in `pyproject.toml`.** Pending: `pip install '.[windows]'` on Windows then run the runner.

**Runner / orchestration:** `scripts/run_baselines.py` reads `tasks/T01_T20.json`, runs each baseline on the same captured screenshot per task, saves an annotated overlay PNG per task to `reports/figures/baselines/<id>.png`, prompts the user once per (task, method) to confirm whether the prediction was correct, and appends rows to **`reports/tables/baseline_results.csv`** with columns: `task, method, found, verdict, xy, bbox, confidence, elapsed_ms, instruction, is_negative, notes`. The CSV is the canonical record; `--auto` re-runs against saved screenshots without re-capturing for reproducibility.
...
- [x] **1.D.1** Aggregate `tables/transfer_experiments.csv` (ML methods) and `tables/baseline_results.csv` (classical) into a single `tables/baseline_summary.csv` with one row per method. **DONE May 6, 2026** via `scripts/analyse_baselines.py`. Per-task pivot also written to `tables/baseline_per_task.csv`. The single combined `tables/method_comparison.csv` (with the additional `VisClick_full_pipeline` row) will be regenerated **after Phase 2** when VisClick's TSR on T01–T15 is known.
- [x] **1.D.2** Phase-1.C bar chart `reports/figures/method_comparison_tsr.png` is generated by `scripts/analyse_baselines.py` (matplotlib). **Scaffolding DONE May 6, 2026; figure regeneration is one command** (`python scripts/analyse_baselines.py [--visclick-tsr 0.X]`). The mAP comparison chart already exists at `reports/figures/method_comparison_map.png` from Phase 1.B.
- [x] **1.D.3** **Observation O20** written in Report §13. Headline: `template = 73.3% TSR (rigid but precise); ocr_only = 33.3% (blind to icons / fooled by repeated text); pywinauto = 6.7% (modern UI frameworks don't expose UIA). VisClick's value proposition is to combine their strengths via YOLO + OCR fallback`. **DONE May 6, 2026.**
...
- [x] **2.1** Run the 15 functional test cases T01–T15 with the **VisClick full pipeline**. **DONE 6 May 2026.** Used `scripts/baseline_visclick.py` (wraps `visclick.detect.Detector` + `visclick.ocr.ocr_box` + `visclick.match.best_box` + `text_ground` fallback) inside the existing Phase-1.C orchestrator (`scripts/run_baselines.py --auto --only-method visclick`), re-using the 15 saved screenshots from Phase 1.C so judging is on identical pixels. **Result: TSR = 11/15 = 73.3%** — same headline as `template`, but with disjoint failure set (template fails on T04/T11/T12 where no template was capturable; VisClick fails on T06 fuzzy-OCR and T13/T15 false-positive paths). 12/15 passes came via the OCR fallback (only 4 detector path successes), confirming Observation O19's recall-ceiling prediction. Full per-task analysis in report §4.7 and §13 O21.
- [x] **2.2** Run the same 15 cases with each Phase-1.C baseline (template / ocr-only / pywinauto). **DONE 6 May 2026** as part of Phase 1.C; merged into `tables/baseline_results.csv` (60 rows = 15 × 4 methods).
- [x] **2.3** Latency NFR (report §10.1): wrote `scripts/run_nfr.py` aggregating elapsed_ms from `baseline_results.csv` into `tables/nfr_performance.csv`. **DONE 7 May 2026.** Used the per-(task,method) wall-times already collected during Phase 1.C / 2.1 rather than a separate timing harness — same workload, same hardware, same judge, no new variance to introduce. Headlines: VisClick p50 = 8 055 ms, p95 = 14 804 ms, max = 23 937 ms (T01 Notepad Save-As, dense-text image); template p50 = 285 ms (28× faster on the 11 tasks where a reference PNG existed); ocr_only p50 = 7 775 ms (statistically tied with VisClick — confirms OCR is the bottleneck). Plan p95 ≤ 15 s NFR target is just-satisfied. Box plot at `reports/figures/nfr_latency_box.png` (regenerate by running `scripts/run_nfr.py` on a machine with matplotlib). See report §10.1 for the full per-task table + dissertation prose.
- [x] **2.4** Six prototype screenshots for report §8.1. **DONE 7 May 2026, 4-of-6 auto-generated.** `scripts/make_prototype_figures.py` reads the saved T01 / T15 screenshots from `samples/test_screenshots/`, runs the ONNX detector + the VisClick matcher, and writes `proto_2_captured.png`, `proto_3_overlay_all.png` (every detection box), `proto_4_picked.png` (matcher's pick), `proto_6_failure.png` (T15 negative-case false-positive in red). Two manual captures remain: `proto_1_terminal.png` (PowerShell + GUI) and `proto_5_after_click.png` (Notepad post-click) — instructions printed by the script. ~5 min on Windows.
