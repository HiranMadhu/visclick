...
- ``TemplateBaseline(Baseline)`` — instance carries the templates
  directory and the correlation threshold.
...
class TemplateBaseline(Baseline):
    """Multi-scale ``cv2.matchTemplate`` baseline."""
    ...
    def __init__(
        ...
        self.templates_dir: Path = Path(templates_dir) if templates_dir else TEMPLATES_DIR
    ...
    def _do_match(
        ...
        """Run TM_CCOEFF_NORMED at three scales (1.0, 0.85, 1.15) and
        pick the best across all scales. Returns ``None`` if best score
        is below the instance threshold.
        """
    ...
    def predict(
        ...
        **_: Any,
        ...
        tdir = Path(templates_dir) if templates_dir else self.templates_dir
        tpl_path = tdir / target_template
        ...
        out, ms = time_call(self._do_match, image_rgb, tpl_rgb)
        r.elapsed_ms = ms
        if out is None:
        ...
        score = out["score"]
        if score < thr:
            r.notes = (f"best score {score:.3f} below threshold {thr:.2f} "
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    tb = TemplateBaseline(threshold=args.threshold)
    r = tb.predict(
