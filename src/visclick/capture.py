...
- ``Capture`` encapsulates the six screen-capture responsibilities as
  instance methods; state is limited to the DPI-awareness flag so repeated
  callers do not pay the Win32 syscall cost.
- The module-level functions (``grab``, ``list_monitors`` etc.) are kept as
  thin delegates to a shared default instance so every existing caller in
  the codebase and the notebooks continues to work unchanged.
"""
...
class Capture:
    """Facade over ``mss`` + Windows DPI plumbing.

    All screen-capture concerns of the pipeline are collected here so that
    downstream layers (Detector, OCR, Bot) never touch ``mss`` directly.
    """
    ...
    def set_dpi_awareness(self) -> None:
        ...
        Without this call, on a 125%/150%-scaled display ``mss`` reports
        virtual (logical) pixels but ``pyautogui.click(x, y)`` expects
        physical pixels — so detected boxes and clicks disagree by 20-30%.
        Calling this once per process makes both libraries see the same
        pixel grid. No-op on non-Windows and idempotent within a process.
        """
        if self._dpi_aware or sys.platform != "win32":
    ...
    def primary_monitor(self) -> dict:
        """Return ``mss.monitors[1]`` as a plain dict (first physical screen)."""
        with mss.mss() as sct:
            return dict(sct.monitors[1])

    def find_pyautogui_primary(self) -> int:
        """Return the ``mss`` monitor index whose size matches pyautogui's primary.

        ``pyautogui.size()`` reports only the Windows primary monitor — the
        one where (0, 0) lives in pyautogui's coordinate system. Capturing
        that same monitor with ``mss`` is the simplest setup: no offset
        needed for clicks. Returns 1 if no match (then ``mss[1]`` *is*
        effectively primary).
        """
        try:
            import pyautogui
    ...
    def grab(self, monitor_index: int = 1) -> Tuple[np.ndarray, int, int]:
    ...
    def save_screenshot(
        self, path: str | Path, monitor_index: int = 1
    ) -> Tuple[int, int, int, int, str]:
        """Capture and save a screenshot. Return ``(width, height, left, top, abs_path)``.

        ``left, top`` are the captured monitor's offset on the virtual
        desktop — needed by callers that want to click at coordinates
        derived from this screenshot.
        """
        img, left, top = self.grab(monitor_index)
        h, w = img.shape[:2]
        path = Path(path)
...
def set_dpi_awareness() -> None:
    _default.set_dpi_awareness()


def list_monitors() -> list[dict]:
    return _default.list_monitors()


def primary_monitor() -> dict:
    return _default.primary_monitor()


def find_pyautogui_primary() -> int:
    return _default.find_pyautogui_primary()


def grab(monitor_index: int = 1) -> Tuple[np.ndarray, int, int]:
    return _default.grab(monitor_index)


def save_screenshot(
    path: str | Path, monitor_index: int = 1
) -> Tuple[int, int, int, int, str]:
    return _default.save_screenshot(path, monitor_index)
