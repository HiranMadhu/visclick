    ...
    def _do_pipeline(self, instruction: str, weights: str, mon_idx: int,
        ...
        try:
            ...
            if pick is None:
                ...
                self._log("  - the model didn't detect that element on this UI")
                self._log("    (try lowering the Conf threshold and re-running)")
                self._log("  - OCR couldn't read the text (try OCR=easyocr)")
                self._log("  - that text isn't on screen at all")
